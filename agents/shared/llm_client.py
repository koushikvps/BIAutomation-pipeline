...
class LLMClient:
    ...
    def enable_rag(self, retriever):
        """Attach a RAGRetriever to automatically ground all LLM calls."""
        self._rag_retriever = retriever
        logger.info("RAG enabled: %d documents in knowledge base", retriever.document_count)
    ...
    def chat(
        ...
        # RAG: inject grounding context from knowledge base
        if self._rag_retriever and self._rag_retriever.document_count > 0:
            rag_context = self._rag_retriever.retrieve_as_context(user_prompt, top_k=15)
            if rag_context:
                user_prompt = (
                    "IMPORTANT: Use ONLY the tables, columns, and definitions from the GROUNDING CONTEXT below. "
                    "Do NOT invent table names or column names that are not listed here.\n\n"
                    f"--- GROUNDING CONTEXT (from client metadata) ---\n{rag_context}\n"
